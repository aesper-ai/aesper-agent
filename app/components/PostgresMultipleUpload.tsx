...
export function PostgresMultipleUpload() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch('http://0.0.0.0:8080/postgres/multiple', {
                method: 'POST',
                body: formData,
            });
    ...
    return (
        <div className="border border-black rounded-lg p-4 mb-6">
            <h2 className="text-xl font-bold mb-4 text-black">Postgres Multiple Files Upload</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    <label htmlFor="postgres-multiple-files" className="block text-sm font-medium mb-2 text-black">
                        Select Multiple Files:
                    </label>
                    <input
                        id="postgres-multiple-files"
                        type="file"
                        multiple
                ...
                >
                    {loading ? 'Uploading...' : 'Upload to Postgres'}
                </button>
            </form>
